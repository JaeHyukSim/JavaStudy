...
public class GetAttractionInfo {
	...
	public static void main(String[] args) {
		...
		GetAttractionInfo g = new GetAttractionInfo();
		// g.getDataFromSite(url);
		// g.attractionVO.display();
		...
		try {
			...
			while (true) {
				try {
					...
					// attractionVO.setReplyList(replyList);
					// String info = attractionVO.display();
					// System.out.println("info : " + info);
					// System.out.println(attractionVO.getTitleV());
					// 파일에 일단 저장해보자
					// saveToFile(info);
				} catch (Exception e) {
	...
	public void getAllLocalHotel(String url) {
		...
		try {
			Document doc = Jsoup.connect(url).get();
			attractionVO.setLocationV(doc.select("h1.bigTitle").text());
			// 1. paging size를 구한다
